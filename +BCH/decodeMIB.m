...
% Outputs:
%   MIB     : a MIB object representing the Master Information Block

    kSSB = kSSB_MSB * 2^4;     % MSB of a 5 bit long kSSB sequence
    commonSCSs = [15 30];
    
    MIB = BCH.MIB;
    MIB.NFrame = bi2de([BCCH_BCH_msg(2:7); SFN_4_LSB] .','left-msb');
    MIB.SubcarrierSpacingCommon = commonSCSs(BCCH_BCH_msg(8) + 1);
    MIB.kSSB = kSSB + bi2de(BCCH_BCH_msg(9:12).','left-msb');
